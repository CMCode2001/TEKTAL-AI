  ...
  if (!idUtilisateur || !utilisateurData || !resultatsAnalyse) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bgCMC p-10">
        ...
        <button onClick={() => navigate('/')} className="mt-4 px-4 py-2 bg-green-600 text-white rounded-md">
          Retour à l'accueil
        </button>
      </div>
    );
      ...
      <div className="max-w-6xl mx-auto space-y-8">
        ...
        <div className="bg-white rounded-xl shadow-lg p-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <p className="text-gray-800"><span className="font-semibold">Niveau d'étude :</span> {utilisateur.niveau_etude}</p>
              <p className="text-gray-800"><span className="font-semibold">Filière :</span> {utilisateur.filiere}</p>
              <p className="text-gray-800"><span className="font-semibold">Domaine d'intérêt :</span> {utilisateur.domaine_interet}</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl shadow-lg p-8">
          ...
          <div className="mb-8">
            <h3 className="text-xl font-semibold text-gray-800 mb-4">Établissements recommandés</h3>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {resultats.etablissements_recommandes.map((etablissement, index) => (
                <div key={index} className="bg-green-50 p-4 rounded-lg">
                  <p className="text-green-800 font-medium">{etablissement}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          {/* Matières requises */}
          <div className="mb-8">
            <h3 className="text-xl font-semibold text-gray-800 mb-4">Matières requises</h3>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {resultats.matieres_requises.map((matiere, index) => (
                <div key={index} className="bg-blue-50 p-4 rounded-lg">
                  <p className="text-blue-800 font-medium">{matiere}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          {/* Parcours possibles */}
          <div className="mb-8">
            <h3 className="text-xl font-semibold text-gray-800 mb-4">Parcours possibles</h3>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {resultats.parcours_possibles.map((parcours, index) => (
                <div key={index} className="bg-purple-50 p-4 rounded-lg">
                  <p className="text-purple-800 font-medium">{parcours}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="bg-yellow-50 p-6 rounded-lg">
            <h3 className="text-xl font-semibold text-yellow-800 mb-4">Message de recommandation</h3>
